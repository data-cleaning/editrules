...
that the reported $x^0_i$ is assumed correct. If $d_i = 1$ the edits 
...
\begin{equation}e^i = 
  ...
  \end{array}
\end{equation}
with $F^i_j \subseteq D_j$ where $D_j$ is the possible set of categories for each $v_j$ and $i = 1 \ldots n$ the number of edits.
...
This means that edit $e^i$ is satisfied is at least one $v_j \in T^i_j$. $e^i$ can therefore be written as:
...
\begin{equation}
   \check{v}_{jk^0} & = & 1 - \delta_j \mbox{ with } k^0 = v^0_j
\end{equation}
It can be easily checked that if $\delta_j = 0$ $\check{v}_{jk^0} = 1$, meaning that the reported value 
$v^0_j$ is assumed correct. If $\delta_j = 1$, $\check{v}_{jk^0} = 0$, meaning that $v_j$ should have a different value.
...
\begin{array}{rcrcl}
  ...
  \sum^m_{j=1} \left(\sum^{d_j}_{k=1} t^n_{jk} \cdot \check{v}_{jk}\right)&& &\geq& 1 \\
    ...
    \check{v}_{1k^0} & +    & \delta_1 & =    & 1 \\
    \ldots  & & \ldots          &       & \ldots \\
    \sum^{d_m}_{k=1} \check{v}_{mk} && & \leq & 1 \\
    \check{v}_{mk^0} & + & \delta_m & =    & 1 \\
\end{array}
\end{equation}

\subsection{The {\sf cateditmatrix} object}
Special form of {\sf editmatrix}. Can be created from an {\sf editarray}.
