...
The default boundary conditions used in {\tt localizeErrors} use ${\rm u_i} = 1000 x^0_i$ and ${\rm l_i} = -1000 x^0_i$.
...
<<keep.source=true>>=
E <- cateditmatrix( 
       expression( if (pregnant) gender == "female"
                 , if (spouse) status %in% c("married")
                 )
                  )
as.data.frame(E)
getAb(E)
