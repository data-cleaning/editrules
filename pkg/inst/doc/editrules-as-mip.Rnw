...
Using principle of Felligi Holt \cite{felligi:1976} which minimizes the weigthed sum of adaptations, the error localization problem can be written as:
...
\end{array}
\end{equation}
with $\rm w_j$ is a weight for variable $x_j$. This problem is equal to the following mixed integer problem
$$
\begin{array}{rcl}
...
The function {\sf localizeErrors} is described in detail in \citep{}. It accepts an {\sf editmatrix} and a {\sf
...
degeneracy. Table \ref{tblErrorLocation} gives an overview of the slots. We extended this method with 
a parameter {\tt method}, that can be used to select the {\tt localizer} or {\tt mip} method.
%
