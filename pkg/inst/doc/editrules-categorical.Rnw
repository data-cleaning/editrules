...
Just like for linear edits, the basic operations on categorical edits are value
substitution and variable elimination. The former amounts to adapting the
datamodel underlying the edit set while the latter amounts to deriving relations
between variables not involving the eliminated variable. 
...
\end{eqnarray}
For two edit sets ${\bf A}$ and ${\bf A'}$, we alse introduce the notation
\begin{equation}
...
also an edit in the sense that a record is invalid if violates it.
...
\citet{fellegi:1976} who first solved the problem of error localization in
categorical data. 



When ${\bf a}_k={\bf a}'_k$, the resulting edit is
...
edits represented by ${\bf A}$ category by category. Suppose that $i$ is the
column index of the first category of $k$.  Collect all pairs of $({\bf
a}^+,{\bf a}^-)$ obeying $a^+_i=\true$ and $a^-_i=\false$. Generate a new set
of edits, consisting of every pair ${\bf a}^+\res_k{\bf a}^-$. By construction,
these new edits all have $a_i=\true$. Iterate for every category of
$k$. There is one exception to take care of. If at any iteration, there are no
edits of type ${\bf a}^+$, the variable cannot be eliminated and in stead, an
empty set is returned. 

The pseudocode of Algorithm {\sf eliminate} elaborates on this scheme by adding
redundancy removal at every step.
...
Given a set of $m$ edits in normal form \eqref{eqDefEdit}:
\begin{equation}
E = \{ e^{(i)} \in \mathcal{P}(D),i\in 1,2,\ldots,m\},
%A_^(i)_1\times
%A_^(i)_2\times\ldots\times
%A_^(i)_n :
\label{eqEditSet}
\end{equation}
...
those properties, it can be shown that the resolution operator is symmetrical,
...
\end{equation}
%
This following property shows that combining an edit with a redundant edit yields a redundant edit.
\begin{lemma}
...
\end{proof}
%
This lemma is important because it shows that no information is lost when
redundant edits are removed in Algorithm \ref{eliminate}.
...
\begin{definition}
Given a a function $f:\mathcal{P}(D)\to\mathcal{P}(D)$. If none of the edits in
$f(E)$ contain variable $k$, we say that $f$ eliminates that variable from $E$.
\end{definition}
%
...
1 &Find all minimal index sets $I_1, I_2,\ldots,I_s$ of $E$, such that $F_k(E[{I_j}])$ eliminates $k$ for every $\{j\in 1,2,\ldots,s\}$.
Here we used the notation $E[{I_j}]=\{e^{ (i)}\in E\,|\,i\in I_j\}$. \\
2 &Generate a new set of edits $E=\ocup_{j=1}^sF_k(E[I_j])$. 
\end{tabular}
\end{center}
%
The following theorem establishes equivalence between Fellegi-Holt elimination
and the resolution operator method of Algorithm  \ref{eliminate}.
\begin{theorem}
