...
In the {\sf editrules} packages, records treated as if every variable is of
{\sf character} type. The boolean representation of edits is mainly used for
storage, and manipulations like variable substitution and elimination.
...
%
\begin{algorithm}[t]
\caption{{\sc subsValue($E$,$k$,$i$)}}
\label{substValue}
\begin{algorithmic}
    \Require an {\sf editarray} $E=\langle{\bf A},{\bf ind}\rangle$, a variable index $k$ and a value $i$
    \State $i \leftarrow \ind(k,i)$
    \State ${\bf A}\leftarrow {\bf A}\backslash\{{\bf a}\in\rows({\bf A})\,|\,a_i=\false\}$
...
\begin{eqnarray}
\lefteqn{
    {\bf a} \res_k {\bf b} = {\bf a}_1\land{\bf b}_1\oplus\ldots\oplus
    {\bf a}_{k-1}\land{\bf b}_{k-1}}\nonumber\\
&\oplus&    {\bf a}_k\lor{\bf b}_k\oplus
    {\bf a}_{k+1}\land{\bf b}_{k+1}\oplus\ldots\oplus
    {\bf a}_n\land{\bf b}_n
\end{eqnarray}
%\begin{eqnarray}
%    {\bf a} \res_k {\bf b} = 
%\left(
%\bigoplus_{i=1}^{k-1}{\bf a}_i\land{\bf b}_i
%\right)
%\oplus
%\left(
%{\bf a}_k\lor{\bf b}_k
...
%\oplus
%\left(
%\bigoplus_{i=k+1}^n{\bf a}_i\land{\bf b}_i
%\right)
%\end{eqnarray}
