...
handling purely categorical data. The {\sf editrules} package is designed to
...
the {\sf R} language.  By providing  basic manipulations, such as error
...
investigate and maintain large sets of categorical or numerical edit sets.  The
variable elimination method seems new to the authors and a novel formulation of
rule manipulation operations in terms of the resolution operator is introduced.
Examples in {\sf R} code are given to assist new users in getting started.
\end{abstract}
...
then $v$ violates $e$}. \index{edit!violation} Conversely, when $r\not\in e$ we say
...
Suppose that $E$ is a set of edits of the form described in Eq.\ \eqref{eqDefEdit}. It is not difficult to verify that an 
...
\ref{corResolution} it must be in ${\bf A}^+(0)\res_k {\bf A}^+(0)$, since
${\bf A}^-(0)\res_k {\bf A}^-(0)$ only generates edits where column number
$\ind(k,1)$ is \false.  Now, consider two edits ${\bf a}^{+}$ and ${\bf
a}^{\prime +}\in{\bf A}^+(0)$.  We have ${\bf a}^+\res_l{\bf
a}^{\prime+}\subseteq {{\bf a}^+\cup {\bf a}^{\prime+}}\subseteq{\bf
A}^+(0)\subseteq{\bf A}(1)$ so each element of ${\bf A}^+\res_k{\bf A}^+$ is
redundant (remember Remarks \ref{editremark} and \ref{remboolnotation}). It
follows from the definition of $\res_k$ that if ${\bf A}(j-1)$ contains every
nonredundant edit with columns \mbox{$\ind(k,1),\ldots,\ind(k,j-1)$} equal to
\true then ${\bf A}(j)$ contains all nonredundant edits where columns
$\ind(k,1),\ldots,\ind(k,j)$ equal \true. Since the algorithm iterates over all
$j\in\ind(k)$ the result follows.
\end{proof}
...
@
The edits state that under-aged cannot be married and that one cannot be a
