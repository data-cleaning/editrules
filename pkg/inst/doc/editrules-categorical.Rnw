...
be written to a file or database. The {\tt data.frame} version contains a
columns with edit names, a character representaition of the edits and a column
for remarks.
...
(rows) violate which edits (columns) It uses the relation of Eq.
(\ref{eqViolatedEdits}) to test the validity of records. 
