...
Univariate edits define a data model. They can be defined with common {\sf R}
syntax using the ${\sf \%in\%}$ operator. If the data model is defined explicitly,
...
editarray}. The $\langle identifier\rangle$ is the name of a predifined {\sf
character} variable storing the unique categories for a variable. In principle,
$\langle identifier\rangle$ may be replaced by any valid {\sf R} expression
evaluating to a {\sf character} vector. However, such constructions are not
...
\end{syntdiag}
Here, each $\langle expression\rangle$ is a logical statement, following
\begin{syntdiag}
\begin{rep}
    \begin{stack}
        \\"!"
...
The example below illustrates the use of seperately defined data models and boolean
categories.
<<keep.source=true>>=
xval <- letters[1:4]
yval <- c(TRUE,FALSE)
...
\begin{syntdiag}
"\""   "!"<expression> \begin{stack}"|"\\"||"\end{stack} <expression>"\""
\end{syntdiag}
Where each $\langle expression\rangle$ is as in the syntax diagram above.  In
...
contrary to the  {\sf if()} form, it allows for vecorized checking of edits,
wich is why it is used internally.

The third and final way to define multivariate edits is simply the shortcut for defining
the null-edit. It follows following syntax diagram:
\begin{syntdiag}
"FALSE"
\end{syntdiag}
This will not commonly be defined by users, but it may turn up when edit sets
are manipulated.
...
The technical background described in Subsections \ref{sscategoryresolotion}
...
disobay some of these rules while the second record is valid. The first
