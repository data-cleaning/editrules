...
%\end{array}
%\end{equation}
%The resolution operator was actually defined earlyer, and independently of the
%current authors by \cite{willenborg:1988}, who also found these properties.
%%
...
%
%\begin{lemma}
%If $E$ is a set of edits, then $\textrm{\sc eliminate}(E,k)$ generates every
%possible nonredundant edit from $E$, with $k$ eliminated.
%\end{lemma}
%\begin{proof}
%Write ${\bf A}(j)$ for the state of {\bf A} at the $j^\textrm{th}$ iteration. We have
%\begin{equation}
%{\bf A}(1) = {\bf A}^+\res_k {\bf A}^-.
%\end{equation}
%By construction we have ${\bf A}(1)=\varnothing$ or ${\bf A}(1)$ has column
%$\ind(k,1)$ equal to \true and ${\bf A}(1)$ contains no redundant edits.
%Suppose another nonredundant edit with this property can be generated from $E$.
%It must be of the form ${\bf a}^+\res_k{\bf a}^{\prime+}$  with ${\bf
%a}^+,{\bf a}^{\prime+}\in{\bf A}^+$  since all combinations ${\bf
%a}^+\res_k{\bf a}^-$ are in ${\bf A}(1)$ and combinations of edits in ${\bf
%A}^-$ cannot generate edits with have column $\ind(k,1)$ equal to \true.
%%If ${\bf a}^{\prime +}\subseteq{\bf a}^+$ or ${\bf a}^{\prime +}\res_k{\bf a}^+=\varnothing$ there is nothing to prove.
%\end{proof}
%
%
%
...
%unimportant because $\res_k$ is symmetric and associative. We may therefore assume without
%loss of generality that $a_1^{(i)}=\false$ for $i\in1,2,\ldots,m$. {\sc
%eliminate} will return $\varnothing$ at the first iteration so all edits in the result contain $k$.
...
%%
%\begin{theorem}
%If $E_1,E_2,\ldots,E_s$ is the minimal irredundant set of subsets of  of $E$ eliminating $k$, then 
%\begin{equation}
%\textrm{\sc eliminate}(\ocup_{j=1}^s E_j,k)=\ocup_{j=1}^s \textrm{\sc eliminate}(E_j,k).
%\end{equation}
%\end{theorem}
%\subsection{An example of variable elimination}
