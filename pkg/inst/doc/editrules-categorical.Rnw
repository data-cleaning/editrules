...
\cite{husson:2010} for an overview]. Howeverm, the options for error
localization and record correction are currently limited.

This paper presents the functionality of the {\sf editrules} package which was
developed to help closing the gap between raw data retrieval and data analyses
with {\sf R}. The paper complements our previous paper \cite{jonge:2011} which
discusses the treatment of numerical data. The algorithms underlying {\sf
editrules}' functionality and the internal representation of categorical data
are discussed as well. The main purpose of the {\sf editrules} package is to
provide a user-friendly environment for handling data restriction rules
(so-called {\em edit rules}), to apply those rules to data, and to localize
erroneous fields in data based on the generalized principle of
\cite{fellegi:1976}. The package does not offer functionality for data
correction although it does facilitate the identification of possible values
...
as well as some of
the algorithms behind the functionality.
