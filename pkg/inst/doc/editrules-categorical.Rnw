...
development of data editing methods which can be automated. Also, since data is
often required to obey many interrelated rules, effective data editing is often
too complex to perform manually. \citet{winkler:1999} mentions practical cases
from statistical offices where each records have to obey 250, 300 or 750
internal consistency checks. Although the {\sf R} statistical environment has
numerous facilities for analyzing categorical data [See {\em e.g.}
\citep{husson:2010}], the options for error localization and record correction
are currently limited.

This paper presents the {\sf editrules} package which is developed to help
...
\cite{fellegi:1976}.  The package does not offer functionality for data but it
facilitates the identification of the set of possible values for erroneous
fields.

This paper complements our previous paper \citep{jonge:2011} which discusses
the treatment of numerical data. We describe the algorithms underlying {\sf
editrules}' functionality and the internal representation of categorical data.
Examples in {\sf R} code are given throughout the text to help new users to
