...
\label{eqDomain}
\end{equation}
where each $D_k$ is a finite set of $d_k$ possible categories for variable $v_k$.
...
$c_k$ in $D_k$ a unique standard basis vector $\vec{\delta}_k(c_k)$ of
...
form}.  A variable $v_k$ is {\em involved} in an edit if $A_k\subset D_k$.
Conversely, we say that $e$ {\em involves} $v_k$ if $v_k$ is involved in $e$. A
variable $v_k$ for which $A_k=D_k$ is not involved in $e$.  Since every
category $c_k$ is mapped to a unique basis vector $\vec{\delta}_k(c_k)$,
...
A record ${\bf v}$ violates an edit if all $v_k\in A_k$, $k=1,2,\ldots n$.  In the
...
$c_k$             & Label for a single category of $D_k$. \\
