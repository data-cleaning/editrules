...
implementation for handeling numerical data while this paper is concerned with
...
invesitgate and maintain large sets of categorical or numerical edit sets.
...
To represent a record $v=(v_1,v_2,\ldots,v_n)$, assign to every category
...
category $c$ of $D_k$ is mapped to a unique basis vector $\vec{\delta}_k(c)$,
edits have a boolean representation $\rho(e)$, given by
\begin{eqnarray}
    e &\xrightarrow{\rho}&
    \Lor_{c\in A_1}\vec{\delta}_1(c)\oplus
    \Lor_{c\in A_2}\vec{\delta}_2(c)\oplus\ldots
    \oplus\Lor_{c\in A_n}\vec{\delta}_n(c),
\end{eqnarray}
...
calculations. For example, an edit contains variable $k$  if and only if the
