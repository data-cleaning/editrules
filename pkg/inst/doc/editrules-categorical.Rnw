...
error correction are currently limited.  The {\sf editrules} package is designed to
...
one is a variable elimination method which is new to the field of data editing.
...
``a man cannot be pregnant'', or ``an under-aged person cannot be
married''.  To facilitate the definition of edit arrays, {\sf editrules} is
equipped with a parser which takes {\sf R}-statements in {\sf character}
or {\sf R expression} format, and translates them to an {\sf editarray}.
...
character} variable storing the unique categories for a variable. An $\langle
identifier\rangle$ can be any {\sf R} expression evaluating to a
{\sf character} or {\sf factor} vector. However, such constructions are not
recommended, since multivariate edit rules depend on a fixed data model.
...
every record in an edit is already a member of a second edit.  In this case,
...
overloaded to search for dublicate edits in an {\sf editarray} directly.  The
...
edits, this amounts to logical resolution. As an example, consider the following syllogism:
...
household} that no matter what value is chosen for that field, the resulting
record will always fall in ${\bf a}^{(1)}$. This shows that this path will
never lead to a valid solution.  We therefore prune the tree here, go up one
node and turn right. 
%%%%%%%%%%%
% T0LR
%%%%%%%%%%
...
nodes to be evaluated from $15$ to $8$ in this example.
...
extensively in \cite{jonge:2011} we will just discuss the example shown in
Figure \ref{RcatErrorLocalizer}.

The data model and edits are defined in Eqs.\ \eqref{eqdom1}-\eqref{eqdom3}. For
{\sf errorLocalizer}, a record must be offered as a named {\sf character}
...
returned, while the {\sf backtracker} object stores some meta-information about the
...
variable elimination and value substitution. Many other functionalities such
as, feasibility checks, and redundancy removal are implemented as well.
Moreover, the package offers functionality to check records against rules and can 
...
data and of interdependent edits.
