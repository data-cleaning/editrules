...
$e$. An edit is redundant if it is the subset of another edit.
...
\label{EqRhoE}
\end{equation}
A simple example is given in Figure \ref{Reditarray}.  A record ${\bf v}$
...
\begin{equation}
\rho({\bf v})'{\bf a} = \sum_{k=1}^n \vec{\delta}_k(v_k)'{\bf a}_k = n.
...
Table \ref{tabBasicfunctions} lists basic functions of {\tt editmatarray}
objects.  The {\tt datamodel} function retrieves the variables and categories
in an edit array, and returns them as a two-column {\tt data.frame}. With {\tt
as.data.frame} or {\tt as.character} one can coerce an editarray so that it
can be written to a file or database. The {\tt data.frame} version contains
a columns with edit names, a character representaition of the edits and a column
for remarks.
...
(rows) violate which edits (columns) It uses the relation of Eqn.
...
%{\tt getArr(E)}              & get array ${\bf A}$\\
%{\tt getInd(E)}              & get index list ${\bf ind}$\\
...
coefficients of ${\bf a}_k$ except the $j$th are set to {\sc false}.  The function {\tt
