...
\end{equation}
The diminsion $d$ of $\rho({\bf v})$ is given by the total number of categories over all variables
\begin{equation}
d = \sum_{k=1}^n d_k.
\end{equation}
When each record in a dataset is represented this way, summing the vectors yields the 
$d$-dimensional vecorized representation of the $d_1\times d_2\times\ldots\times d_n$ contingency
table of the dataset (this is sometimes called the complete disjunctive table). 
...
are used to indicate a combination of objects.{\tt editarray} is the
central object for computing with catgegorical edits, just like {\tt
...
It is both tedious and error prone to define and maintain an edit array by hand.
In practice, categorical edits are usually stated verbosely, such as: ``if the
gender is male, then you cannot be pregnant'', or ``if you are under-aged, you
