...
\label{sIntroduction}
Many applications in statistics involve the analysis of categorical data. These
analysis are often hindered by occurrences of incomplete or inconsistent raw
...
Institutes spend up to 40\% of their resources on this process
\citep{waal:2011}. For this reason a lot of attention is paid to the
development of data editing methods which can be automated. Also, since data
are often required to obey many interrelated rules, effective data editing can
be too complex to perform manually. \citet{winkler:1999} mentions practical
cases where records have to obey 250, 300 or even 750 internal consistency
rules.  Although the {\sf R} statistical environment has numerous facilities
for analyzing categorical data [See {\em e.g.} \citet{husson:2010}], the
options for error localization and record correction are currently limited.
...
correction but it facilitates the identification of the set of possible values
for erroneous fields.

Under the hood, the package has several innovations with respect to the
...
algorithm which allows for on-the-fly redundant rule removal. This algorithm
appears to be new to the field of data editing. To facilitate the description
of algorithms and their properties, a formulation of categorical edit
manipulations in terms of the so-called resolution operator is introduced in
this paper as well. This formulation allows for elegant algebraic proofs of
...
data \citep{jonge:2011}. We describe the algorithms underlying
{\sf editrules}' functionality and the internal representation of categorical
data.  Examples in {\sf R} code are given throughout the text to help new users
