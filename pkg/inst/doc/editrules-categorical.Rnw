...
implementation for handeling numerical data while this paper is concerned with handling
...
redundancy checks, this package offers a toolset which may be used to
...
\begin{equation}
D_k = \{c_k\in D_k\,|\,c_k=1,2,\ldots,d_k\}.
...
d_2\times\ldots \times d_n$ contingency table must be zero. A record is {\em
...
$c_k$ in $D_k$ a unique standard basisvector $\vec{\delta}_k(c_k)$ of
$\mathbb{B}^{d_k}$.  The boolean representation $\rho(v)$ of the full
record is the direct sum
%
\begin{equation}
    v \xrightarrow{\rho} \vec{\delta}_{1}(v_1)\oplus\vec{\delta}_{2}(v_2)\oplus\ldots\oplus\vec{\delta}_n(v_n),
\end{equation}
...
category $c_k$ (of $D_k$) is mapped to a unique basis vector $\vec{\delta}_k(c_k)$,
edits have a boolean representation $\rho(e)$, given by
\begin{eqnarray}
    e &\xrightarrow{\rho}&
    \Lor_{c_1\in A_1}\vec{\delta}_1(c_1)\oplus
    \Lor_{c_2\in A_2}\vec{\delta}_2(c_2)\oplus\ldots
    \oplus\Lor_{c_n\in A_n}\vec{\delta}_n(c_n),
\end{eqnarray}
...
with $c_k\in 1,2,\ldots,d_k$, we use the following notations:
\begin{eqnarray}
\label{eqInd1}
    {\bf ind}(k,c_k) &=& \sum_{l< k }d_l + c_k\\
    {\bf ind}(k)   &=& \{{\bf ind}(k,c_k)\,|\, c_k\in D_k\}.
\label{eqInd2}
\end{eqnarray}
So $\ind(k,c_k)$ is the column index in {\bf A} for variable $k$ and category $c_k$
...
\begin{quote}
{\sf <abbreviated var. name><separator><abbreviated cat. label>}, e.g.
{\sf gnr:feml} for gender female
\end{quote}
...
For convinience, the function {\sf datamodel} accepts an {\sf editarray} as
...
The character expressions that can be read by {\sf editarray}, are either ${\sf \%in\%}$ statements
of the form 
\begin{quote}
\verb+<variable> %in% <expression>+
\end{quote}
to define the data model or {\sf if}-statements of the form
\begin{quote}
\verb+"if ( <condition> ) <expression>"+ 
\end{quote}
to define rules relating variables. The expressions follows standard {\sf R}
syntax, which should be already familliar to the reader. 

Note (see Figure \ref{Reditarray}) that double quotes are used to enclose the
whole expression, while single quotes are used for category names. Table
\ref{tabOperators} shows which operators and functions can be used to specify
categorical edit rules.

Categories may be {\sf character} or {\sf logical} literals.  It is worth
noting that expressions on the right hand side of the {\sf \%in\%}  and {\sf
==} operators are evaluated. One useful application of this is that the
categories, or data model can be defined outside of the edits:
%
<<keep.source=true>>=
xval <- letters[1:4]
yval <- c(TRUE,FALSE)
...
isFeasible(M)
@
The explanation is that $e_1$ and $e_2$ contradict eachother only when 
$x={\sf a}$, so
<<keep.source=true>>=
...
one untill either no edits are left or an obvious contradiction is found. Eliminating
all variables amounts construcing the solution of an error localization problem
...
elimination method.  Finally, in subsection \ref{sseliminateexample} we given
an example of eliminating variables with the {\sf editrules} package. Readers
not interested in the technical background can skip subsections
\ref{sscategoryresolution} and \ref{ssequivalence}.
...
substValue} function, which accepts an {\sf editarray}, a variablename and a
...
concering $gender={\sf male}$ may be deleted, so here only the datamodel is left with
...
\begin{eqnarray}
   {\bf v}\in {\bf a}\res_k{\bf a}' &\Rightarrow& \rho({\bf v})\in{\bf a} \lor {\bf v}\in{\bf a}'\\
   {\bf v}\in {\bf a} &\Rightarrow& {\bf v}\in{\bf a}\res_k{\bf a}' \lor {\bf a}\res_k{\bf a}'=\varnothing
    \label{eqRedundant}
\end{eqnarray}
...
defined edits is called an {\em implied edit}. \index{edits!inplied} If the
...
\index{editarray@{\sf editarray}!variable elimination}
The purpose of the {\sf eliminate} function is to derive all possible nonredundant
...
$P_2$ & People who are not married cannot be a marriage partner\\
\hline
$C$ & Under aged people cannot be a marriage partner.\\   
\\
\end{tabular}\\
Here, the conclusion $C$ is derived from premisses $P_1$ and $P_2$ by eliminating
{\em marital status}. In {\sf editrules} the above example can be performed as
...
E <- editarray(c(
    ...
    "if (maritalStatus != 'married') positionInHousehold != 'partner'"
))
@
...
localize the errors:
\begin{equation}
v = ({\rm married},{\rm under\:aged},{\rm partner }).
\end{equation}
...
representations of $e_1$ and $e_2$ respectively. The record and edits are denoted in boolean
representation as shown in Example \ref{datamodel}. 
The weight $w$ counts the
...
solution where {\em age} has to be altered to fix the record, and indeed changing {\em age}
from {\sf under aged} to {\sf adult} will make the full record valid. Note that
...
report containing degeneracies, solution weights runtimes and wether the maximum runtime
...
control over the error localization process since it allows to set different
parameters for each record. This can be useful, for example when 
reliability weights are calculated for each record.

The {\sf errorLocalizer} function is described extensively in \cite{jonge:2011}, so here
we will discuss the example shown Figure \ref{RcatErrorLocalizer}.

The data model and edits are the same as in Figure \ref{RlocalizeErrors}. The difference
here is that a record must be offered as a named {\sf character} vector. A call to
{\sf errorLocalizer} generates a {\sf backtracker} object which contains all information
necessary to start searching the binary tree. After calling {\sf \$searchNext()}
the weight and first found solution are returned, while the backtracker object stores some
meta-information about the process, most significantly the duration of the search.
A second call yields a better solution and the third call returns {\sf NULL}, indicating
that all minimal weight solutions have been found.
%
\begin{Rcode}
<<keep.source=TRUE>>=
# Define a record 
...
%\clearpage
\section{Conclusions}
This paper descibes the theory and implementation of categorical edit
...
elimination algorithm wich seems to be new in the field of data editing.
