...
%\end{corollary}
%\begin{proof}
%This follows from the fact that ${\sf eliminate}(E,k)$ generates all possible
%nonredundant edits from $E$ with $k$ eliminated (Theorem \ref{eliminates}).
%\end{proof}
...
edits, this amounts to logical resolution. For example, suppose we have the following
rules

\begin{tabular}{lm{0.8\textwidth}}
\\
$P_1$ & Under aged people cannot be married\\
$P_2$ & If you are not married you cannot be a marriage partner\\
\hline
...
the marital status variable. In {\sf editrules} the above example can be performed as
...
parameters for each record. This can be usefull, for example when 
reliability weights are calculated for each record.

The errorLocalizer function is described extensively in \cite{jonge:2011}, so here
