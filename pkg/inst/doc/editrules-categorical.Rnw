...
\title{Manipulation of categorical data edits and error localization with the {\tt editrules} package\\
...
A categorical data record $\bf v$ is defined as an element of the cartesian product space $D$:
\begin{equation}
...
${\bf v}\in e$ then $\bf v$ violates $e$}. In data editing literature, such
...
d_n$ contingency table must be zero.
...
manipulation infeasible. The {\sf editerules} package offers functionality to
...
can be obeyed. Fellegi and Holt's principle should be considered as the last
...
In the next section we describe the representation of edits and records 
as implemented in the {\sf editrules} package. Readers who are just interested
in the practical examples can skip Section \ref{sectBoolrep}.
...
To represent a record ${\bf v}$, assign to every category $i$ in $D_k$ a unique
standard basisvector $\vec{\delta}_k(i)$ of $\mathbb{B}^{d_k}$.  The boolean
representation $\rho({\bf v})$ of the full record reads
%
\begin{equation}
...
$A_k\subset D_k$.  Conversely, we say that $e$ {\em contains} $v_k$ if $v_k$ is
involved in $e$. A variable $v_k$ for which $A_k=D_k$ is not contained in
$e$. 
...
an edit contains variable $k$  if and only if the inner product ${\bf 1}_{d_k}'{\bf a}_k<d_k$,
...
\begin{equation}
\sum_{k=1}^n \vec{\delta}_k(v_k)'{\bf a}_k =\rho({\bf v})'{\bf a} =  n.
...
\begin{eqnarray}
    ...
    \lefteqn{\textrm{or}}\nonumber\\
    \label{eqRedundancy2}
    e   &\subseteq& \tilde{e}\textrm{ with  } \tilde{e}\in E.
\end{eqnarray}
In \eqref{eqRedundancy1}, $e$ is redundant since it cannot contain any records.
It can be tested by checking if any ${\bf 1}_{d_k}'{\bf a}_k=0$.  In the case
of \eqref{eqRedundancy2}, $e$ is redundant because any edit violating $e$ also
violates $e'$. Using $\rho(e)={\bf a}$ and $\rho(\tilde{e})=\tilde{\bf a}$, this can be
tested by checking if ${\bf a}\land\tilde{\bf a}={\bf a}$ or equivalently if ${\bf
a}\lor \tilde{\bf a}=\tilde{\bf a}$.
...
and the $d_k$ denote the number of categories of each variable.  The object
{\bf ind} is a {\sf list} which relates columns of {\bf A} to variable names
and categories. The brackets are used to indicate a combination of objects.
The {\tt editarray} is the central object for computing with catgegorical
edits, just like the {\tt editmatrix} is the central object for computations
with linear edits.
...
to screen, a {\tt character} version of the edits is shown as well, for
readability.
}
\label{Reditarray}
\end{Rcode}
...
Table \ref{tabBasicfunctions} lists basic functions of {\tt editarray} objects.
The {\tt datamodel} function retrieves the variables and categories in an edit
array, and returns them as a two-column {\tt data.frame}. With {\tt
as.data.frame} or {\tt as.character} one can coerce an editarray so that it can
...
be switched off. The result of {\tt as.data.frame} version contains columns
with edit names, a character representaition of the edits and a column for
remarks.

The function {\tt violatedEdits} takes an {\tt editarray} and a {\tt
data.frame} as input and returns a logical matrix indicating which record
(rows) violate which edits (columns). It works by parsing the {\tt editarray}
to R-expressions and evaluating them within the {\tt data.frame} environment.
By default, the records are checked against the data model. This can be turned
off by providing the optional argument {\tt datamodel=FALSE}. 
...
{\tt as.data.frame(E)}       & coerce edits to {\tt data.frame}\\
{\tt contains(E)}            & which edits contains which variable\\
{\tt as.character(E)}        & coerce edits to {\tt character} vector\\
{\tt violatedEdits(E,x)}     & check which edits are violated by {\tt x}\\
{\tt isObviouslyRedundant(E)}& find redundancies [Eq.\ \eqref{eqRedundancy1}], duplicates\\
{\tt duplicated(E)}          & find duplicate edits\\
{\tt isSubset(E)}            & find edits, subset of another edit [Eq.\ \eqref{eqRedundancy2}]\\
{\tt isObviouslyInfeasible(E)}& detect contradictions\\ 
{\tt substValue(E,var,value)} & substitute a value\\
\hline
\end{tabular}
\end{threeparttable}
\end{table}
...
{\tt isObviouslyRedundant}. By default, this funciton also checks for duplicate edits,
but this may be turned off. The function {\tt duplicated} is overloaded from the standard
{\sf R} function and the function {\tt isSubset} detects which edits are a subset or
...
coefficients of ${\bf a}_k$ except the $j$th are set to {\sc true}, so $k$ is
not contained in the edits anymore.  The function {\tt substValue} can be used
to substitute one or more values in an {\tt editarray}.
