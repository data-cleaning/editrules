...
\label{smixeddata}
The term ``mixed data'' is used in this paper to indicate data containing both numerical
...
A conditional edit that can be treated with the {\sf editrules} package can
be written canonically as:
\begin{eqnarray}
\label{eqedit1}
\lefteqn{
\textrm{\sf if } {\bf v}\in F \Land {\bf Ax}\boldsymbol{\odot}{\bf b} \textrm{ \sf then FALSE} 
\textrm{, where }}\\
&&\begin{array}{l}
...
\end{array}
\end{eqnarray}
which includes all types of conditional edits as used in \cite{waal:2003}.
Here, $F$ is used to denote a set of categorical edits, as defined in
\cite{loo:2011b} and $\bf A \odot b$ denotes a set of linear numerical edits as defined in 
\cite{jonge:2011}.
Note that this is a canonical form. All permutations of categorical and numerical edits in the premise are 
equivalent. Furthermore each term in the premise can be moved to the consequent in negated form. We 
therefore exclude equality restrictions {\sf ==} from $\bf A$ (as a precise reader may have noticed), 
because the logical negation of a linear equality yields a
nonlinear edit. This prohibits certain operations such as variable elimination
by the Fourier-Motzkin procedure. In practice, the lack of the {\sf ==} operator in conditional
edits poses no real restriction since any equality can be written as two
inequalities.

Internally, conditional edits are represented by replacing each (numerical) linear condition $k$
with a dummy variable $t_k$, which is defined as
\begin{equation}
\label{eqdummy}
t_k = \left\{\begin{array}{l}
\true\textrm{ when } {\bf a_k\cdot x}\odot b_k\\
\false\textrm{ when }{\bf a_k\cdot x}\notodot b_k.
\end{array}\right.
\end{equation}
%
...
\end{equation}
where $t_1\cdots t_m$ correspond to the $m$ linear restrictions of $\bf A$ in Eq.\ \eqref{eqedit1}.
Eq.\ \eqref{eqdummyedit1} can be stored in a boolean representation  as described 
...
The linear restrictions $\bf Ax \odot b$ for all conditional edits can be
gathered in one matrix $\bf \hat{A}$ and can be stored as described in \cite{jonge:2011}. 
...
the {\sf condition} attribute and $E_{\sf mixnum}$ exclude each other.
...
Assuming a truth value for a conditional numerical edit amounts to substituting a
...
disjunct convex sets where all conditional numerical edits are (re)moved  (See
