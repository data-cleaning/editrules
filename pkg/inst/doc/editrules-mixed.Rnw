...
the mixed-integer approach to a seperate paper \citep{jonge:2012}.
...
truth values of the linear restrictions occuring in conditional edits.
...
numerical and not occuring in $E_{\sf mixnum}$ or categorical variables. These
...
occuring in $E_{\sf mixnum}$ (and possibly $E_{\sf num}$), the following steps
must be performed.
%
\begin{enumerate}
