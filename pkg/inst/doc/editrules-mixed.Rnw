...
condition $y>0$ may be added the numerical edits (here: $x+y=z$) and the conditional edit
removed. On the other hand, substituting $x = -3$ gives $\textrm{\sf  if } -3 > 0 \textrm{ \sf  then } y>0$,
so the conditional edit can be deleted, as is done by {\sf substValue}.
<<>>=
substValue(F,"x",-3)
@
