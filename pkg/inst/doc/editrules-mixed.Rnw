...
data and conditional restrictions. A fourth paper \citep{jonge:2012} is dedicated
...
In the practice of (official) statistics, data records are often required to obey 
various restrictions, including sum rules, positivity demands or other linear
inequalities, and categorical restrictions which exclude certain value combinations.
...
the mixed-integer approach to a separate paper \citep{jonge:2012}.
...
$E_{\sf mixnum}$ is an {\sf editmatrix}, holding numerical edits occurring
in conditional edits. The rownames of {\bf mixnum} correspond to 
dummy variables, occurring in the categorical edits stored as 
...
all variables not occurring in any edit and all categorical
...
\cite{loo:2011b} for a syntax diagram of possible set expressions.
...
independent graphs shown here. This graph is displayed using the {\sf igraph}
...
generate a graphical display of this connectivity. Figure \ref{figgraph} shows
the connectivity graph between the variables and edits of Figure
\ref{figeditfile}.
...
To be able to manipulate conditional edits, it is convenient to separate them
by eliminating the dummy variables. This is possible by making assumptions
about their truth values and processing the consequences. As an example,
consider the following set of edits.
%
\begin{equation}
...
leave of the tree corresponds to one complete set of assumptions about the
truth values of the linear restrictions occurring in conditional edits.

Here, making an assumption about such a truth value amounts to substituting a
dummy variable in the $E_{\sf mixnum}$ part of an {\sf editset}, copying the
assumption to a separate conditional {\sf editmatrix} attribute of the {\sf
editset}, and updating $E_{\sf mixcat}$ and $E_{\sf mixnum}$ accordingly.  The
{\sf substValue} function, which will be discussed in more detail in Section
\ref{sssubstitution}, is equipped to do this. The assumptions which have become
an attribute of the resulting {\sf editset}, are returned by {\sf condition}.
...
functions {\sf isFeasible} on objects of class {\sf editmatrix} and {\sf
...
expensive, so some short-circuiting is done in the implementation of lines
\ref{algp1} and \ref{algp2} to avoid unnecessary computations.
...
object called $E_{\sf num}$.  To substitute a value in an editset, four cases
are distinguised.
...
occurring in $E_{\sf mixnum}$ (and possibly $E_{\sf num}$), the following steps
must be performed.
%
\begin{enumerate}
...
variables holds.  If a dummy variable is substituted, the {\sf substValue}
moves the corresponding numerical edit from $E_{\sf mixnum}$ to the {\sf
condition} attribute of the {\sf editset}. Next, $E_{\sf mixcat}$ is updated
accordingly. Substituting dummy variables explicitly is not normally done by
end users, as the names and values of dummy variables remain hidden to them.
The {\sf condition} attribute is stored as an {\sf editmatrix} and can be
requested with {\sf condition}.
...
described in \cite{loo:2011b} and \cite{loo:2012}. Consequently, the result of
an {\sf eliminate} operation is an object of class {\sf editlist}. 

As an example, consider again the set of edits of
Eqs.~\eqref{eqedits} and~\eqref{eqeditlist}. In Figure~\ref{figprojections},
the valid regions are indicated as grey areas. 
Geometrically, eliminating variable $y$ amounts to projecting along the
$y$-axis when $y$ is eliminated or along the $x$-axis when $x$ is eliminated.
In Figure \ref{figprojections} the projections are indicated with bold lines.
Algebraically, we need to apply Fourier-Motzkin elimination on the second set
of linear edits of each element of Eq.~\eqref{eqeditlist} to obtain
\begin{displaymath}
E = \left(
...
of fields in a records that can be adpted or imputed, such that no edits are
...
editset}) starts by first separating the edits into blocks and disjunct sets
...
editlists} have to be treated differently. Remember that an editlist is a set
...
editmatrices} and -{\sf arrays}, however the difference is that each
...
unfeasible it is removed from the list. When no feasible regions are left and
...
solutions equals the weight of previous solutions. At that point, all other
...
with $n$ the number of variables and depth of the tree. However, in some cases
the tree depth can be reduced by noting that variables which violate simple,
single-variable range edits will always be part of the error localization
solution. These single-range edits are easily checked (in a vectorized manner)
and variables violating such edits can be removed from the multivariate
localization problem causing a considerable speedup in some cases.
...
construct such a matrix, we assume a set of single-valued range edits, and
...
variable $j$ and $0$ otherwise. The matrix $M$ is then given by
\begin{equation}
{\bf M} = {\bf V}{\bf C}.
\end{equation} 
...
accepts an editset and a {\sf data.frame} along with some optional arguments and
returns an object of class {\sf errorLocation}. The funciton {\sf errorLocalizer} returns
...
optional parameter {\sf method="mip"} to {\sf localizeErrors}. For a comparison between the two algorithms refer to
\cite{jonge:2012}.
