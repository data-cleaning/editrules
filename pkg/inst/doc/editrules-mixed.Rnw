...
\begin{algorithmic}[1]
    ...
    \Else
        ...
        \If {$\textrm{\sf isFeasible}(E_{\bf num})\land \textrm{\sf isFeasible}(E_{\bf mixcat})$}
...
<<fig=true,echo=false>>=
v <- disjunct(E)
par(oma=c(0,0,0,0),mar=c(0,0,1.5,0),mfrow=c(2,2))
for ( i in 1:4 ){
    set.seed(1)
    plot(
        v[[i]],
