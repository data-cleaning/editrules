...
Figure \ref{figprojections} (in gray) the valid areas in the $x$-$y$ plane,
...
set of {\sf editset} objects.  Every editset gaines an extra attribute called
...
extensively here, in stead we refer the reader to \cite{jonge:2011} and
