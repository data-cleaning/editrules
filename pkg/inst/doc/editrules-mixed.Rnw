...
implemented using two computational methods. The implementation first uses a
branch-and-bound algorithm while the second implementation depends on a
mixed-integer formulation of the problem.  This paper discusses the
branch-and-bound algorithm, leaving treatment of the mixed-integer approach to
a separate paper \citep{jonge:2012}.
...
In the next section we describe how edits can be read in to {\sf R}. 
Section \ref{ss:editset} gives a technical description of the underlying
{\sf editset} object. Section \ref{ss:editlist} describes how conditional 
edits can be disentagled and \ref{ss:checking} discusses basic operations
on edits and data.
...
The example file of Figure \ref{fig:editfile} can be read as follows:
<<>>=
...
{\sf editset}. When printed to screen, the data model for categorical variables,
...
\end{array}
\end{eqnarray}
Here, it is understood that every linear inequation in the premisse must be
obeyed for a record to be valid.  Also, $F$ is used to denote a (set of)
categorical edit(s), as defined in \cite{loo:2011b}. This formulation includes
all types of conditional edits as used in \cite{waal:2003}. This form is
canonical, in the sense that any condition in the premisse can be moved to the
consequent in negated form. This is also the reason that no linear {\em
equalities} are allowed in either premisse or consequent: a negated equality
yields a nonlinear numerical edit.

In practice, one will define simpler edits, with one rule in the premisse and one
rule in the consequent, for example:
\begin{equation}
\textrm{\sf if } v\in e \textrm{ \sf then } {\bf a}\cdot{\bf x}\odot b,
\end{equation}
where $e$ is a categorical edit, {\bf a} is a real vector and $b$ is a constant.
More examples are given in Figure \ref{fig:editfile}.

Internally, conditional edits are represented by representing each (numerical)
linear condition $k$ with a dummy variable $t_k$, which is defined as
\begin{equation}
\label{eqdummy}
...
\end{array}\right.
\end{equation}
where each ${\bf a}_k$ is a row of {\bf A}.
...
Eq.\ \eqref{eqedit1}.  Eq.\ \eqref{eqdummyedit1} can be stored in a boolean
...
independent graphs shown here. (Displayed using the {\sf igraph0}
package of \cite{csardi:2006}.)}
\label{figgraph}
\end{figure}
%
%
As mentioned in the introduction, edits often are interrelated because an edit may
...
To manipulate conditional edits, we first separate them by generating all valid
combinations of truth values for the dummy variables described in the previous
section. This way, all connections between numerical and categorical variables
disappear at the cost of generating multiple disjunct {\sf editsets} which may
...
Figure \ref{figprojections} (page \pageref{figprojections}) shows (in gray) the
...
a record must obey the rules defining either the left, or the right gray area.
...
generating all possible assumptions about the truth values of conditional numerical edits. 
Each leaf of the tree corresponds to one complete disjunct set of assumptions about the
truth values of the linear restrictions occurring in conditional edits.

Assuming a truth value for a conditional numerical edit amounts to substituting 
...
and updating $E_{\sf mixcat}$ and $E_{\sf num}$ accordingly.  The {\sf
...
valid record is in precisely one of the convex regions defined by the resulting set of
...
not ({\sc false}). For an {\sf editlist} ({\sf editenv}) a list of boolean vectors
...
The {\sf S3} method of {\sf isObviouslyInfeasible} for objects of class {\sf
...
\label{sselimination}
Variable elimination amounts to deriving all implicit edit rules from a set of
edits which do not contain the eliminated variable anymore.  On an {\sf
editset} object, it is executed by first separating the edits in disjunct
sets as described in Section
\ref{sseditlist}).  Next, since in each element of the resulting {\sf editlist} the
dummy {\sf editmatrix} $E_{\sf mixnum}$ is empty, variable elimination takes
place on each $E_{\sf num}$ or $E_{\sf mixcat}$ of the {\sf editlist} for
...
apply Fourier-Motzkin elimination on the second set of linear edits of each
element of Eq.~\eqref{eqeditlist} to obtain
\begin{displaymath}
E = \left(
...
\subsection{Error localization with {\sf localizeErrors} and {\sf
errorLocalizer}} The interfaces of {\sf localizeErrors} and {\sf
...
extensively here, in stead we refer the reader to \cite{jonge:2011a} and
