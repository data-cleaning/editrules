...
\true\textrm{ when } {\bf a}_k{\bf \cdot x}\odot b}_k\\
...
gathered in one matrix and stored as described in \cite{jonge:2011}. 
In {\sf editrules} linear restrictations are stored in an object of 
class {\sf editmatrix}. 

To store conditional edits, along
