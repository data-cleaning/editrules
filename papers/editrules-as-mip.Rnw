...
\begin{equation*}
\begin{array}{l}
  \Minimize_{(p,c,\Delta_p,\Delta_c)\in\mathbb{R}^2\times\{0,1\}^2} \Delta_p + \Delta_c\\
...
$m$ and \emph{Age class} $a$. We have
\begin{equation*}
...
Using Eq.\ \eqref{eq:ecat} we get $\mathbf{e}(F)=(0,1,0,1)$ and one may verify
that $\mathbf{e}(F)^T\mathbf{d}(\textsf{married, child})=0$. For $\mathbf{v}^0$, the MIP
problem for error localisation now looks like this.
\begin{equation*}
\begin{array}{l}
  \Minimize_{(m,a,\Delta_m,\Delta_a)\in D\times \{0,1\}^2} \Delta_m + \Delta_a\\
