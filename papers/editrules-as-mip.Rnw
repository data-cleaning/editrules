...
\begin{equation}e^j = 
  \begin{array}{rl}
    \label{eq:catedit}
    \mbox{{\bf if }} & v_i \in F^j_i \mbox{ for }i = 1, \ldots, m \\
    \mbox{ {\bf then}} & \mbox{FALSE}
  \end{array}
\end{equation}
with $F^j_i \subseteq D_i$ where $D_i$ is the possible set of categories $\{c_{i,1}, \ldots, c_{i,n_i}\}$ for each $v_i$ and $j = 1 \ldots n$ the number of edits.

Remarkable about categorical edits is that its literature (\ref) formulates the 
problem negatively. If a condition is true than the edit is violated. For mip 
we need the possitive formulation, since it requires constraints 
that must hold.

Equation (\ref{eq:catedit}) can be written in the following form:
\begin{eqnarray}
     e^j  & = & \lnot \bigwedge_{i=1}^m v_i \in F^j_i = \bigvee_{i=1}^m\lnot (v_i \in {F^j_i}) \\
          & = &  \bigvee_{i=1}^m v_i \not\in {F^j_i} = \bigvee_{i=1}^m v_i \in T^j_i \label{eq:catpos}
\end{eqnarray}
with $T^j_i = D_i \setminus F^j_i$. 
This means that edit $e^j$ is satisfied if at least one $v_i \in T^j_i$. 

\subsection{Mip formulation}
Recall from section \ref{sec:mipproblem} that in a mip problem ${\bf x}$ is a 
numerical vector. We therefore encode each $v_i$ in $n_i$ binary dummy variables
$$
v_i = (d_{i,1} \ldots d_{i,n_i})
$$
with
\begin{equation}
  d_{i,k} = \left\{ 
...
\begin{equation}
  {\bf x} = (d_{1,1}\ldots d_{1,n_1}, \ldots, d_{m, 1}, \ldots, d_{m, n_m}, 
  \Delta_1, \ldots, \Delta_m)
\end{equation}

Constraint matrix $A$ for categorical edits is composed out of two parts.
...
Second the categorical edits, equation \ref{eq:catpos} can now be written as:

\begin{equation} \label{eq:tmip}
  \sum^m_{i=1} \left(\sum^{n_i}_{k=1} t^j_{i,k} \cdot d_{i,k}\right) 
  \geq 1
\end{equation}
...
\begin{equation} \label{eq:aijk}
  ...
  \begin{array}{ll}
         0 & \mbox{if } c_{i,k} \not\in T^j_i\\
         1 & \mbox{if } c_{i,k} \in T^j_i
  \end{array} \right. 
\end{equation}
If one of the $c_{i,k} \in T^j_i$ then the edits is valid.\\


The soft constraints $A^0$ can be written as:

\begin{equation}
   d_{i,k^0_i}  =  1 - \Delta_i  \mbox{ with } v^0_i = c_{i,k^0}
\end{equation}

It can be easily checked that if $\Delta_i = 0$ $d_{i,k^0_i} = 1$, 
