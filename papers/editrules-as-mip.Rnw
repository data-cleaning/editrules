...
\left[\begin{array}{rrr|r}
  \multicolumn{2}{c}{ \mathbf{A}^H} & \multicolumn{1}{c|}{\mathbf{0}} & \mathbf{b}^H\\
  \mathbbm{1} & \mathbf{0} & M\mathbbm{1}  &\mathbf{x}^0\\
  \mathbf{0}  & -\mathbbm{1} & -M\mathbbm{1} &-\mathbf{x^0}
\end{array}\right] 
\end{equation}
...
\section{Usage}
\marginpar{Toevoegen: hoe kunnen \code{M} en \code{epsilon} worden ingesteld.}
