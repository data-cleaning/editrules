...
\end{equation}
where each $D_i$ is a finite set of categories for the $i^\textrm{th}$ categorical variable.
An edit $e_j$ is a subset of $D$ that can be written as
\begin{equation}
e_j = F_{j1}\times F_{j2}\times\cdots \times F_{jm},
\end{equation}
where each $F_{ji}$ is a subset of $D_i$. It is understood that if a record
$\mathbf{v}\in e_j$ then the record violates $e_j$. Hence, categorical edits are
...
edits $\overline{e_j}$, such that if $\mathbf{v}\in\overline{e_j}$ then $\mathbf{v}$ satisfies $\mathbf{e_j}$.
Here, $\overline{e_j}$ is the complement of $\mathbf{e_j}$ in $D$, which can be written as
%
\begin{eqnarray}
\lefteqn{
\overline{e_j} = \overline{F}_{j1}\times D_{2}\times\cdots \times D_{m}
}\nonumber\\
&\cup& D_{1}\times \overline{F}_{j2}\times\cdots \times D_{m}\cup\cdots 
\cup D_{1}\times D_{2}\times\cdots \times \overline{F}_{jm}
\label{eq:complement}\\
&=& \overline{F}_{j1} \cup \cdots \cup \overline{F}_{jm},
\end{eqnarray}
%
where for each variable $i$, we have $\overline{F}_{ji}=D\backslash F_{ji}$. Observe
that it follows from Eq.\ \eqref{eq:complement} that if at least one $v_i\in \overline{F}_{ji}$,
then \mathbf{v} satisfies $e_j$.
