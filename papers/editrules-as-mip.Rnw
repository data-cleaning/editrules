...
%\Big),
\end{equation}
\todo[inline]{This is not correct, because or-ing all records 
creates $e(v_i) = \D_i$ in many cases. And that results in that the edit is 
always true!\\
It really is:
\begin{equation}
...
or (ab)using a previous notation
\begin{equation}
...
\todo[inline]{This is not correct!, since in many cases 
$\overline{F_i} = \emptyset \implies d(v_i) = 0$ and thus $\mathbf{e}$ can not
be the set of all records satisfying the 
edit...
}
