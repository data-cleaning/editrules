...
An edit $e$ is a subset of $D$ that can be written as
\begin{equation}
e = F_{j1}\times F_{j2}\times\cdots \times F_{jm},
\end{equation}
where each $F_{ji}$ is a subset of $D_i$. It is understood that if a record
$\mathbf{v}\in e$, the record violates $e$. Hence, categorical edits are
...
edits $\overline{e}$, such that if $\mathbf{v}\in\overline{e}$ then $\mathbf{v}$ satisfies $\mathbf{e}$.
Here, $\overline{e}$ is the complement of $\mathbf{e}$ in $D$, which can be written as
%
\begin{eqnarray}
\lefteqn{
\overline{e} = \overline{F}_{1}\times F_{2}\times\cdots \times F_{m}
}\nonumber\\
&\cup& F_{1}\times \overline{F}_{2}\times\cdots \times F_{m}\cup\cdots 
\cup F_{1}\times F_{2}\times\cdots \times \overline{F}_{m},
\label{eq:complement}
\end{eqnarray}
%
where for each variable $i$, we have $\overline{F}_i=D\backslash F_i$. Observe
that it follows from Eq.\ \eqref{eq:complement} that if at least one $v_i\in \overline{F}_i$,
then \mathbf{v} satisfies $e$.
