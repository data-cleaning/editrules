...
\lnot(\mathbf{v}\in \overline{F}) \land \lnot(\mathbf{a}^T\mathbf{x}\leq b)\}
\end{equation}
...
\begin{eqnarray}
&&\lnot(\mathbf{v}\in \overline{F}) \land \lnot(\mathbf{a}^T\mathbf{x}\leq b)\nonumber\\
&\Leftrightarrow& \lnot (\mathbf{v}\in \overline{F} \lor \mathbf{a}^T\mathbf{x}\leq b)\nonumber\\
&\Leftrightarrow& \lnot (\mathbf{v}\in F \Rightarrow \mathbf{a}^T\mathbf{x}\leq b).
...
Line 3 and 4 try to restrict \code{age}  to $9$ and line 5 tries to restrict 
\code{married} to \code{TRUE}. The last line shows the objective function, which
canbe influenced by supplying \code{weight} to \code{as.mip}.
