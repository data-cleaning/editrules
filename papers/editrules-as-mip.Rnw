...
as.mip(E)
@
Above example shows the rewriting of ${\bf A}^H$. If we add the $A^0$
constraints we get:
<<tidy=FALSE>>=
as.mip( E
      , x=c(gender="male", pregnant=TRUE)
      , weight=c(1,2)
      )
@
...
as.mip(E, x=c(age=9, married=TRUE))
@
...
A advantage of the branch and bound algorithm is that it can generate all best
