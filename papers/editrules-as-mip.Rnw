...
<<echo=FALSE>>=
em <- editrules:::writeELAsMip(E, x=c(x=10, y=0), weight=c(x=1,y=2))
@
...
<<>>=
mip <- editrules:::as.mip(E)
@
Above example show the rewriting of ${\em A}^H$. If we add the $A^0$
constraints we get:
<<>>=
editrules:::as.mip(E, x=c(), weight=c())$E
@
