...
A set of edits splits $D$ into a region where records are considered valid and
records where records are considered invalid. Specifically, each categorical edit
$e$ can be written as 
\begin{equation}
e = F_{1}\times F_{2}\times\cdots \times F_{j},
\end{equation}
...
negatively formulated (they specify the region of $D$ where $\mathbf{v}$ may not
be) in contrast to linear edits which are positively formulated (they specify
the region of $\mathbb{R}^n$ where $\mathbf{x}$ must be). To be able to translate
categorical edits to a MIP problem, we need to reformulate them as positive
edits $\overline{e}$, such that if $\mathbf{v}\in\overline{e}$ then $\mathbf{v}$ satisfies $e$.
Here, $\overline{e}$ is the complement of $\mathbf{e}$ in $D$, which can be written as
%
\begin{eqnarray}
\label{eq:invedit}
\lefteqn{
...
\end{eqnarray}
%
where for each variable $i$, we have $\overline{F}_{i}$ is the complement of
$F_{i}$ in $D_i$.  Eq.\ \eqref{eq:complement} states that if at least one
$v_i\in\overline{F}_{i}$, then $\mathbf{v}$ satisfies $e$. 

To be able to formulate the Felligi Holt-problem in terms of a MIP problem, we first
associate with each categorical variable $v_i$ a binary vector $\mathbf{d}$ the coeffiecients
of which are defined as follows (also see Eq.\ \eqref{eq:defineD}).
\begin{equation}
...
\end{array}\right.
\end{equation}
Thus, each element of $\mathbf{d}(v_i)$ corresponds to one category in $D_i$. It
is zero everywhere except at the value of $v_i\in D_i$. We will write $\mathbf{d}(\mathbf{v})$
to indicate the concatenatenated vector $(\mathbf{d}(v_1),\ldots,\mathbf{d}(v_m))$.
Similarly, for each edit
$e$ we can define a binary vector $\mathbf{e}$ representing an edit as
\begin{equation}
\mathbf{e}(e) = \Lor_{\mathbf{v}\in \overline{e}}\mathbf{d}(\mathbf{v}),
\end{equation}
where the $\lor$ is to be interpreted elementwise.

The demands in Eq.\ \eqref{eq:invedit} can now be stored as linear restrictions:
\begin{equation}
\label{eq:hardcat}
\mathbf{e}(e)\cdot \mathbf{d}(\mathbf{v}) \geq 1,
\end{equation}
while the fact that each variable $v_i$ can contain maximal a single value translate to 
\begin{equation}
\label{eq:hardmaxcat}
\sum_{j=1}^{|D_i|}d_j(v_i) = 1.
\end{equation}
Equations \eqref{eq:hardcat} and \eqref{eq:hardmaxcat} constitute the hard restrictions, stored in
$\mathbf{R}^H$ of Eq. \eqref{eq:errlocasmip}.

The vector to minimize over can now be written as
\begin{equation}
\mathbf{z} = (\mathbf{d}(\mathbf{v}),\Delta_1,\Delta_2,\ldots\Delta_m).
\end{equation}
Since the variables $v_i$ can take only a single value, the restriction matrix $\mathbf{R}^H$ will
at least contain the following restrictions.
