...
\begin{equation}
\mathbf{v} \in \mathbf{\overline{F}}= \bigvee_{i} v_i \in \overline{F}_i
\end{equation}
...
\subsection{Mixed type edits rules}
...
Eigenlijk kunnen we alle statements van de vorm
...
We now define a binary variables $l_i$ that indicate whether $\mathbf{x}$ is 
