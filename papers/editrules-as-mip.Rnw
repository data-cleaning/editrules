...
\mathbf{v}\in \mathbf{F} \land \lnot(\mathbf{a}^T\mathbf{x}\leq b)\}
\end{equation}
where it is understood that $\F \subseteq \D$ and if $\mathbf{r}
\in \FD$, then $\mathbf{r}$ violates edit $e$.
To obtain a positive formulation $\FDc$, we first negate the set membership condition 
and apply basic
rules of first-order logic:
\begin{eqnarray}
&&\lnot\big(\mathbf{v}\in \F \land \lnot(\mathbf{a}^T\mathbf{x}\leq b)\big)\nonumber\\
&\Leftrightarrow& \lnot( \mathbf{v}\in \F) \lor \mathbf{a}^T\mathbf{x}\leq b\nonumber\\
&\Leftrightarrow& (\mathbf{v}\in \F \Rightarrow \mathbf{a}^T\mathbf{x}\leq b).
...
\end{equation}
\todo[inline]{Da's inderdaad de standaard form, maar deze dekt geen restricties van de volgende
vorm:
if ($x>0$) then ($y>0$)
die we ook aankunnen.
Eigenlijk kunnen we alle statements van de vorm
\begin{equation}
\bigvee_i v_i \in \overline{F}_i \bigvee_j \mathbf{a}_j^T\mathbf{x} \leq b_j
\end{equation}
aan. Ik twijfel of we dat in het stuk moeten verwerken: ik neig naar wel, maar dan
moet het hierboven (weer...) herschreven worden.
} 
...
are found in are found e.g. in business statistics and energy statistics. 
The numerical
...
We can introduce an error by setting an $x_i$ to $-1$.
