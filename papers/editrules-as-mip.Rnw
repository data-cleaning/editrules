...
\begin{itemize}
  ...
  \item The objective function that minimizes the (weighted) number of adaptions in the form of ${\bf w}^T {\bf \Delta}$. The object function is the same for all formulations.
\end{itemize}
...
\end{equation}
Note that for reasons we give in a following section you should avoid writing strict inequalities in practice.
...
\subsection{Benchmark comparison to {\tt backtracker}}
