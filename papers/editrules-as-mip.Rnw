...
Mixed integer programming is well understood and there are several sofware packages that implement efficient solvers for linear and mixed integer problems. If your problem can be formulated as a mip problem, mip software can be used to solve the original problem.
In {\sf editrules} we use \ref{lpSolveAPI:2011} to formulate and solve our mip problems. \\

Most mip software allows for a broader (but equivalent) formulation of the mip problem.  (\ref{eq:mipmax}) can be written as  a minimization problem (by changing the sign of ${\bf}c^T$),  $A$ can include equality constraints (by replacing it withtwo inequility constraints) and ${\bf x}$ can be negative (by adding so-called slack variables). We will make use of these broader definition.

In this paper we formulate Felligi Holt error localization \citep{fellegi:1976} for numerical, categorical and mixed constraints as a mip problems. This error localization principle minimizes the (weigthed) number of variables that should be changed to obey the current constraints. The formulations for error localization for the three types are different but all have the same parts:
\begin{itemize}
  \item The objective function that minimizes the (weighted) number of adaptions in the form of ${\bf w}^T {\bf \Delta}$, with ${\bf w}$ the vector of weights and ${\bf \Delta}$ a vector of binary adaptation variables. If $\Delta_i = 1$, value $r_i$ must be adapted. The objective function is the same for all formulations.
  \item A numerical description ${\bf x}$ of record ${\bf r}$ that includes binary adaptation variables ${\bf \Delta}$ and other dummy variables. 
  \item The constraints (edits) that each record ${\bf r}$ {\bf must} obey formulated in the form ${\bf A} {\bf r} \leq {\bf b}$
  \item The reported values ${\bf r}^0$ that record ${\bf r}$ {\bf should} have formulated as soft constraints ${\bf A}^0 {\bf x} = {\bf r}^0$
  \item Binary adaption variable(s) $\Delta_i$ that are constrained to 1 if $r_i \neq r^0_i$ in the form ${\bf A}^\Delta {\bf x} \leq {\bf b}^\Delta$.
\end{itemize}
...
\end{equation}
with $\Delta_i \in \{0,1\}$, and auxilliary variables $\delta^{+}_i \geq 0 $ and $\delta^{-}_i \geq 0$.\\
...
\begin{equation}
a_{ij} r_i < b_j \implies a_{ij} r_i \leq b_j - \epsilon
\end{equation}
...
\begin{eqnarray}
                         r_i & =    & r^0_i + \delta^{+}_i - \delta^{-}_i  \label{eq:numsoft} \\
  \delta^{+}_i + \delta^{-}_i& \leq & M \Delta_i                           \label{eq:numadapt}
\end{eqnarray}
with $M$ sufficiently large and $i = \{1,\ldots,m\}$. \\

Equation \ref{eq:numsoft} allows $r_i$ to be different from $r^0_i$, but equation \ref{eq:numadapt} shows
that this results in $\Delta_i = 1$. It can easily be checked that if $\Delta_i = 0$ then $x_i = r^0_i$ and 
if $\Delta_i = 1$ then $x_i$ can take any value 
between $r^0_i - M$ and  $r^0_i + M$. 
Since the objective function tries to minimize $w^T \Delta_i$, the mip solver will try to keep each variables $r_i$ at its reported value $r^0_i$.

This makes the mip formulation for error localization in numerical edits complete.

\subsection{Implementation}
{\sf editrules} uses an {\sf editmatrix} object to store numerical data constraints. This object is described extensively in {\cite{jone:}}
...
which by default uses a branch and bound algorithm. We extended this function with a parameter {\tt method}, that can be used to select the {\tt "localizer"} or {\tt "mip"} method. 
...
A categorical record $\bf r$  with reported values $({r^0_1,\dots,r^0_m)}$ has the following $n$ constraints:
\begin{equation}e_i = 
    \label{eq:catedit}
    \mbox{{\bf if }}  \Big( \bigwedge_{j=1}^m r_j \in F_{ij} \Big)
    \mbox{ {\bf then }} \mbox{FALSE}
\end{equation}
with $F_{ij} \subseteq D_j$ where $D_j$ is the possible set of categories $\{1, \ldots, k_j\}$ for each $r_j$ and $i = 1 \ldots n$ the number of edits.

\subsection{Mip formulation}
Recall from section \ref{sec:mipproblem} that in a mip problem ${\bf x}$ is a numerical vector. We encode each $r_j$ in $k_j$ binary dummy variables $(d_{j1} \ldots d_{jk_j})$ 
\begin{equation}
  \mbox{with }
  d_{ji} 
  =  \left\{ 
...
The numerical encoding of record ${\bf r}$ with $m$ categorical variables is 
$$
  {\bf x} = \big( d_{11},\ldots,d_{1k_1}, \ldots, d_{m1},\ldots,d_{mk_m}, \Delta_1, \ldots, \Delta_m \big)
$$


Constraints (\ref{eq:catedit}) can be written in the following form:
\begin{eqnarray}
     e_i  & = & \lnot \bigwedge_{j=1}^m r_j \in F_{ij} = \bigvee_{j=1}^m\lnot (r_j \in F_{ij}) \\
          & = &  \bigvee_{j=1}^m r_j \not\in F_{ij} = \bigvee_{j=1}^m r_j \in T_{ij}
\end{eqnarray}
with $T^i_j = D_j \setminus F^i_j$. 
This means that edit $e_i$ is satisfied if at least one $r_j \in T_{ij}$. $e_i$ can now be written as:
...
x <- list(pregnant=TRUE, gender="male")
mipform <- editrules:::writeELAsMip(E,x)
print(mipform$E, textOnly=TRUE)
@
...
x <- list(age=9, married=TRUE)
editrules:::writeELAsMip(E, x)
@
\section{Conclusion}
