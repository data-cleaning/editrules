...
Membership to $\mathbf{\overline{F}}$ can also be 
written with the following logical statement:
\begin{equation}
\label{eq:cat_logical}
\mathbf{v} \in \mathbf{\overline{F}}= \bigvee^m_{i=1} v_i \in \overline{F}_i
\end{equation}
...
The above relation can be interpreted as stating that 
$\mathbf{e}$ is the set of all categories that make a record satisfy $e$.
...
$\mathbf{D}$ that is disallowed while linear edits define regions in $\mathbb{R}^n$ that
are allowed. We may choose a negative formulation of edits containing
both variable types by defining $\mathbb{F}$: 
\begin{equation}
\mathbb{F} = \{\mathbf{r}\in \mathbf{D} \times\mathbb{R}^n: 
\mathbf{v}\in \mathbf{F} \land \mathbf{x} \in \Ls \}
\end{equation}
where $\F \subseteq \D$ and $\Ls$ is a convex subspace of 
$\mathbb{R}^n$ defined by $\mathbf{A}^\Ls\mathbf{x} > \mathbf{b}^\Ls$.
It is understood that if $\mathbf{r}
\in \FD$, then $\mathbf{r}$ violates edit $e$. The rows of 
$\mathbf{A}^{\Ls}\mathbf{x} > \mathbf{b}^\L$ define half spaces $L_i$ and their 
intersection forms $\Ls$.

To obtain a positive formulation $\FDc$, we first negate the set membership condition 
and apply basic
rules of first-order logic:
\begin{eqnarray}
&&\lnot\big(\mathbf{v}\in \F \land \mathbf{x} \in \Ls \big)\nonumber\\
&\Leftrightarrow& \lnot \big( \mathbf{v}\in \F \land \mathbf{x} \in L_1 \ldots \land \mathbf{x} \in L_{|\Ls|})
\nonumber\\
&\Leftrightarrow& \mathbf{v} \not\in \F \lor \mathbf{x} \not\in L_1 \ldots \lor 
\mathbf{x} \not\in L_{|\Ls|}\\
&\Leftrightarrow& \mathbf{v}\in \Fc \lor \mathbf{a}^T_{1}\mathbf{x}\leq b_{1} 
\lor \ldots \lor \mathbf{a}^T_{|L|}\mathbf{x}\leq b_{|L|}.
\label{eq:implication}
\end{eqnarray}
This then yields a positive formulation of $\FDc$ given by
\begin{equation}
  \label{eq:mixedtype}
  r \in \FDc = \bigvee_i v_i \in \overline{F}_i \bigvee^{|\Ls|}_{j=1} \mathbf{a}_j^T\mathbf{x} \leq b_j
\end{equation}
...
Wat doen we met de standaardform?
\begin{equation}
...
by \citet{waal:2003}, which it restricted to half space $L_1$.
\todo{Give example of expressiveness?}
It is the form in which the \code{editrules} package
accepts mixed-type edits and has the property that if
$\mathbf{r}\in\FDc$ then $\mathbf{r}$ satisfies $e$.

Note that this formulation is also a generalization of both the linear and the
categorical edit. In case $\Ls = \emptyset$ Eq. (\ref{eq:mixedtype}) is equal to 
a categorical edit (Eq. (\ref{eq:cat_logical})).
When $\F = \emptyset$ and $\Ls = L_1$, we have a linear edit.

We now define binary variables $l_i$ that indicate whether $\mathbf{x}$ is 
in half space $L_i$ or not:
\begin{equation}
l_i = \left\{\begin{array}{l}
0 \textrm{ when } \mathbf{a}^T_i\mathbf{x}\leq b_i\\
1 \textrm{ when } \mathbf{x} \in L_i
\end{array}\right.
\end{equation}
...
\begin{equation}
\label{eq:mixedit}
\mathbf{e}^T\mathbf{d}(\mathbf{v}) + \sum^{|\Ls|}_{i=1} (1-l_i) \geq 1.
\end{equation}
These form the user-defined part of the $\mathbf{R}^H$ part of the restriction matrix.
To connect $l_i$ with the linear restrictions we also add
\begin{equation}
\label{eq:linconsequent}
\mathbf{a}^T_i\mathbf{x} \leq b_i + Ml_i,
\end{equation}
to $\mathbf{R}^H$ with $M$ a suitably large positive constant. Indeed, if the
$l_i=0$, the inequality $\mathbf{a}^T_i\mathbf{x}\leq b_i$ is enforced. When 
$l_i=1$ it
...
\code{as.mip} implements the mip formulation of sections \ref{sec:mipproblem}.
We illustrate this with a small example of a mixed edit.
...
($=1$). If \code{married==TRUE} than restriction of line 2 turns into 
\code{age >= 17}.
...
will signify if these variables need to be adapted.
...
to $9$, but to $[9-10^7, 9+10^7]$
Line 5 tries to restrict 
\code{married} to \code{TRUE}. The last line shows the objective function, which
