...
of numbers that enter the mip problem. In the \code{lpsolve} library (as well
as in R), both numbers on the real line are represented as \code{double
precision} numbers, meaning that there are about 15 decimal positions of
precision.



\marginpar{Hier zouden we de defaults moeten verantwoorden en ook uitleggen waarom we bepaalde default lp.control opties hebben gekozen}
