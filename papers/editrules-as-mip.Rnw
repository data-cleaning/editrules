...
\end{equation}
It is now not difficult to see that the demand (Eq.\ \eqref{eq:complement}) that at 
...
Using the above notation for $\mathbf{v}$, and adding the $\Delta$-variables
