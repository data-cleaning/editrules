...
ignored. The high-level error localization detects and exploits this block
structure, so users need not concern themselves with it directly. However, to
facilitate edit rule investigation and maintenance, the lower-level {\sf
blocks} function is exported to user space. This function returns a {\sf list}
of the independent {\sf editsets}, as illustrated by the following example.
<<>>=
length(blocks(E))
@
Clearly, the three independent graphs from Figure \ref{fig:graph} have been recognized.

The largest cluster of edits in Figure \ref{fig:graph} connects numerical variables with
categorical variables. For example, via the edit labeled \verb"mix11". Operations such
as variable elimination are difficult to implement for such edit sets. However, it is
possible to split up such a set further by working out what happens when we assume
statements in the premise or consequent to be {\sc true} or {\sc false}. 
...
plot(E)
@
\caption{Dependency graph of edits defined in Figure \ref{fig:editfile}. The squares represent 
edit rules, and the circles represent variables. An edge indicates that a variable occurs in an
edit.
}
\label{fig:graph}
\label{fig:graph}
...
\verb"mix9" can be dropped, since the value of $y$ does not matter anymore.
Combined with \verb"num4", assuming that $x\leq0$ this means that $x=0$.  These
two assumptions exclude each other (they cannot both hold) and yield two
different edit sets. If we continue making assumptions for the numerical
statements in conditional edits recursively and work out their consequences, we
get a list of {\em conditional} {\sf editsets} where for each {\sf editset},
the dependences between categorical and numerical edits have been severed.

The function {\sf disjunct} does exactly this. It speeds up computation by
detecting whenever contradictory assumptions have been made. The function
...
localization) are can be performed with  standard methods for numerical or
categorical data.  The downside is that for large, strongly connected edit
sets, separating them into disjunct sets can be a computationally daunting
task, growing exponentially in the number of edits. For this reason, we have
implemented an error localization method not requiring variable elimination
explicitly on which we will report in a separate paper \citep{jonge:2012}.

Finally, we note that the utility function {\sf separate} performs both the block 
decomposition based on variable occurrence and calls, when appropriate, the
{\sf disjunct} function on conditional edits. The results are simplified as
...
numerical or categorical edits, we refer to \cite{jonge:2011} and
\cite{loo:2011b}.
