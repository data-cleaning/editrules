...
when the restriction in the consequent resolves to \false, the premise cannot
...
As an example, we will eliminate variable $y$ from the following set of edits.
\begin{equation}
...
violated anymore. Given a record violating a (subset of) edits from an {\sf
editset}. For each field, we may assume that it's value is either right and
should be kept, or wrong and should therefore be altered. The consequences of
such assumptions on the remaining edits can be worked out: assuming that a
field holds a correct value corresponds to substituting the variable in the
editset with that value. Assuming that a field should be altered corresponds to
eliminating that variable. Indeed, eliminating a variable corresponds to
deriving all nonredundant edits not pertaining to the eliminated variable.

The solution to an error localization problem corresponds to a sequence of
variable substitutions and eliminations on a set of edits. \cite{waal:2003}
devised a branch-and-bound algorithm to efficiently generate such sequences
in a binary tree while pruning branches which yield infeasible solutions.
The function {\sf localizeErrors} uses a variant of De Waal's algorithm to
solve the error localization problem. 

In some cases, especially when many equivalent solutions exist, the
branch-and-bound algorithm can be slow. Therefore, in {\sf editrules}, error
localization starts by separating edits into blocks using the {\sf blocks}
function discussed in Section \ref{ss:disentangling}. Separate blocks share no
variables and are treated independently. Blocks consisting of only numerical or
only categorical edits are treated as described in \cite{loo:2011b} and
\cite{jonge:2011}.

Blocks consisting of conditional edits have to be treated differently.  


They are split up in independent sets, using the {\sf disjunct} function,
described in \ref{ss:disentangling}. 


The resulting list of {\sf editsets} is
treated by the branch-and-bound algorithm simultaneously. Whenever one of the
{\sf editsets} becomes unfeasible, it is removed. If the last editset has
become infeasible, a branch is pruned.  The process is accelerated by detecting
whenever the current weight of partial solutions equals the weight of previous
solutions. At that point, all remaining values are substituted to check whether
the branch can lead to an actual solution. If so, the solution is stored; if
not, a bound condition is hit. 

Finally, a third optimization is performed prior to entering the branch-and-bound
algorithm
